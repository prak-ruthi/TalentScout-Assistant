...
def get_gemini_3_response(prompt, api_key):
    try:
        # Latest 2026 SDK Client
        client = genai.Client(api_key=api_key)
        
        # Using Gemini 3 Flash with Minimal Thinking for instant speed
...
if st.session_state.step == "greeting":
    ...
    st.info("This tool uses Gemini 3 Flash to generate tailored technical assessments.")
    if st.button("Start Screening"):
        st.session_state.step = "info_gathering"
        st.rerun()

# 2. INFO GATHERING (Your Updated Version)
    ...
    with st.form("info_form"):
        ...
        if submitted:
            # Assignment requirement: Ensure all fields are filled
            if not all([name, email, phone, position, location, tech]):
                st.error("⚠️ Please fill in all details required by TalentScout.")
            else:
                st.session_state.candidate_data = {
                    "name": name, 
                    "tech": tech, 
...
# 3. QUESTION GENERATION
    ...
    st.write(f"### Assessment for {st.session_state.candidate_data['name']}")
    st.write(f"**Position:** {st.session_state.candidate_data['position']}")

    if st.session_state.questions is None:
        with st.spinner("Gemini 3 Flash is generating high-quality questions..."):
    ...
    if st.button("Finish & Exit"):
        st.success("Screening Data Saved!")
        st.session_state.clear()
        st.rerun()
